(ns babyagi.main
  "Kotodama WASM entrypoint for the Babyagi clean-room actor (L4) — Clojure port.
  ...
  facts (`babyagi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "babyagi")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"babyagi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "babyagi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :babyagi-compat/ready)
